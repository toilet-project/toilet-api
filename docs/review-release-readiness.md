...
- 기존 독립 검증 저장소의 `review-anonymization-v1` orphan 브랜치에 건수·해시·세대만 포함한 0건 기준선을 만들었다. 회원 파기 main/history는 수정하지 않았다.
...
- `ReviewUnlinkRestore`는 검증된 snapshot만 받아 오프라인 DB에서 재처리한다. 기존 복원 실행기 후보에 연결했고 GitHub Actions의 일회용 MySQL 8에서 회원 파기와 리뷰 연결 해제의 결합 재적용·반복 실행 0건·보존 행 검사가 통과했다. 운영 미니 PC에는 복원 컨테이너를 만들지 않는다.
...
- [x] 전용 작성자 연결 해제 대장 초기화와 운영 미니 PC 읽기 전용 0건 사전 검사. 실제 복원 훈련은 별도 장비/유지보수 WBS로 분리.
...
구체적인 순서와 플래그는 [리뷰 운영 활성화 실행안](review-production-activation-runbook.md)에 고정했다. 웹의 다중 운영 빌드 게이트, 전용 대장 0건 기준선, 읽기 전용 사전 검사와 가상 결합 복원은 준비됐다. 정책 공개·시행 시각 승인, 배포 직전 백업, 최종 이미지의 마운트 후 snapshot 재검사와 main 병합·순차 배포는 아직 실행하지 않는다.
