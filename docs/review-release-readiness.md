...
- [ ] 전용 작성자 연결 해제 대장 초기화와 운영 미니 PC 읽기 전용 사전 검사. 실제 복원 훈련은 별도 장비/유지보수 WBS로 분리.
...
구체적인 순서와 플래그는 [리뷰 운영 활성화 실행안](review-production-activation-runbook.md)에 고정했다. 웹의 다중 운영 빌드 게이트와 비배포 후보 CI는 준비됐지만, 실제 리뷰 대장 초기화·결합 복원 시험과 정책 공개가 남아 있으므로 위 체크는 아직 완료로 표시하지 않는다.
