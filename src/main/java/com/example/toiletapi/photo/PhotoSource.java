...
public record PhotoSource(URI uri) {
    public static PhotoSource from(String provider, Map<String,Object> attributes) {
        if (!"kakao".equals(provider)) throw new IllegalArgumentException("Unsupported provider");
        Object account = attributes.get("kakao_account");
        if (!(account instanceof Map<?,?> a) || !(a.get("profile") instanceof Map<?,?> p)
                || !Boolean.FALSE.equals(a.get("profile_image_needs_agreement"))
                || Boolean.TRUE.equals(p.get("is_default_image"))) return new PhotoSource(null);
        Object value = p.get("profile_image_url");
        ...
        boolean trusted = host != null && java.util.Set.of("k.kakaocdn.net","img1.kakaocdn.net","t1.kakaocdn.net").contains(host);
